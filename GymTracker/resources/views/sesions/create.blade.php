...
<form action="{{ route('sesions.store') }}" method="POST">
  ...
  <div class="mb-4">
    ...
    <input
      ...
      name="fecha"
      value="{{ old('fecha') }}"
      class="border p-2 w-full @error('fecha') border-red-500 @enderror"
      required
    >
    @error('fecha')
  ...
  <div class="mb-4">
    ...
    <textarea
      name="nota"
      class="border p-2 w-full @error('nota') border-red-500 @enderror"
    >{{ old('nota') }}</textarea>
    @error('nota')
      <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
    @enderror
  ...
  <div id="series-container">
    <template id="serie-template">
      <div class="flex space-x-2 mb-2 series-item">
        ...
        <button
          type="button"
          onclick="this.closest('.series-item').remove()"
          class="text-red-500"
        >✕</button>
      </div>
    </template>
